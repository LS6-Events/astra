...
func TestInlineFunctionsAreMapped(t *testing.T) {
	...
	// Path parameter
	require.Equal(t, "param", paths.Path("/inline/{param}.get.parameters.0.name").Data().(string))
	require.Equal(t, "path", paths.Path("/inline/{param}.get.parameters.0.in").Data().(string))
	require.True(t, paths.Path("/inline/{param}.get.parameters.0.required").Data().(bool))
	require.Equal(t, "string", paths.Path("/inline/{param}.get.parameters.0.schema.type").Data().(string))
	// Query Parameter
	require.Equal(t, "query", paths.Path("/inline/{param}.get.parameters.1.in").Data().(string))
	require.Equal(t, "name", paths.Path("/inline/{param}.get.parameters.1.name").Data().(string))
